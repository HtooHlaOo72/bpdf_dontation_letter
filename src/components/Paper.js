...
const Paper = (props) => {
  ...
  let { _id, donor, amount, amountText, unit, signedBy, topic, date, serialNo, supply } = data;
        ...
        <div className="col-md-8 mx-auto text-center d-flex justify-content-center align-items-center">
          <div ref={cert} id="paper" className={css.cert}>
            ...
            <p className={css.maintext} id="main_text">
              ...
              အသက်နှင့်ရင်းကာ တိုက်ပွဲဝင်လျှက်ရှိသော
              <br /> ပြည်သူ့ကာကွယ်ရေးတပ်ဖွဲ့ (ပဲခူးတိုင်း) <span className="fw-bolder">
                BPDF
              </span>{" "}
              အတွက်
              <br />
              ...
              <span className={css.student}>
                ...
                {type === "money" && (
                  <b>
                    {amount ? amount.toLocaleString("en-US") + "/-" : "0"} {unit ? unit : "MMK"}
                    <br />
                    {amountText ? `(${amountText})` : "(မြန်မာငွေကျပ် ဆယ်သိန်းတိတိ)"}
                  </b>
                )}
                {type === "supply" && <b>{supply}</b>}
              </span>
              <br />
              ပေးပို့လှူဒါန်းခြင်းအား ဂုဏ်ယူ ဝမ်းမြောက်စွာ မှတ်တမ်းတင်အပ်ပါသည်။
            </p>
            ...
            <h6 className="uniq-id my-5">
              အမှတ်စဉ် - {serialNo ? convertId(serialNo) : "00001"} ( {_id} )
            </h6>
          </div>
        </div>
      </div>
